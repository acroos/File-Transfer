...
int main(int argc, char **argv)
{
    ...
    // Prompt user to enter a place for the file to be copied.
    fprintf(stderr, "Please enter a filename for file to be received at\n") ;
    if (fgets(userInputFileName, PATH_MAX, stdin) == NULL) {
    ...
    while( 1 )
    {
        ...
        buffer = (byte *)malloc (receiveSize * sizeof(byte)) ;
		...
		if (packetNum == 0) {
			...
			file = (byte *)malloc(fileSize * sizeof(byte)) ;
            fprintf(stderr, "\n") ;
    		...
    		// fprintf(stderr, "Checksum straight from buffer: ") ;
    		// for(u_int i=(receiveSize-2); i<receiveSize; i++) {
    		// 	fprintf(stderr, "%02x ", buffer[i]) ;
    		// }
    		// fprintf(stderr, "\n") ;

    		u_short incomingCheckSum, calculatedCheckSum ;
    ...
    outFile = fopen(userInputFileName, "wb") ;
...
Packet processPacket(byte * packetContents, u_int length) {
	Packet recv ;
	byte * message = (byte *)malloc((length-2) * sizeof(byte)) ;
	byte * chksum  = (byte *)malloc(2 * sizeof(byte)) ;
	u_int j = 0 ;
    u_int i = 0 ;
...
u_short * bytes2ushorts(byte * byteArray, u_int length) {
	u_short * shortArray = (u_short *)malloc(length * sizeof(u_short)) ;
    u_long i = 0 ;
